...
def sc_done(sid):
    ...
    if(len(MANAGER.DONE) == MANAGER.CONFIG["PROTOCOL"]["N"]):
        ...
        if(MANAGER.CONSENSUS_MESSED):
            ...
            tra1 = MANAGER.CONTRACT.functions.calculateTrust().transact()#LAST IS MANAGER.
            tx_receipt = MANAGER.CONN.eth.wait_for_transaction_receipt(tra1)
            GAS_USED += tx_receipt["gasUsed"]
        ...
        for x in range(0,MANAGER.CONFIG["PROTOCOL"]["N"]):
            w = MANAGER.CONTRACT.functions.reputation(x).call()
            print(w)
            weight[x] = w
...
class Manager:
    ...
    def data_preparation(self,PATH,NUM_PARITIONS,ENCRYPTION,IPFS_UPLOAD,UNIFORM,VARIENCE):
        ...
        PARTITIONS[-1].to_csv("partitioned_data/validation.csv",index=False) #Creating CSVs            
        copyfile("partitioned_data/validation.csv",self.BASE+"/part_data/"+str(y)+".csv")
    ...
    def reputation_round(self):
        ...
        temp= self.TEST_DATA.iloc[rand(9,108)]
        temp = temp.values.reshape(1,-1)
        self.GROUND_TRUTH.append(temp.tolist()[0][0])
        ...
        sio.emit("start_consensus",{"Data":temp.tolist()[0]},room="experts")
...
MANAGER.data_preparation(MANAGER.CONFIG["METADATA"]["TRAIN_DATA_PATH"],MANAGER.CONFIG["METADATA"]["NUM_PARTIONS"]+1,
                  MANAGER.CONFIG["METADATA"]["ENCRYPTION"],False,
                  MANAGER.CONFIG["METADATA"]["UNIFORM"],VARIENCE=50)
with open("./donepart", 'w') as f:
    f.write('')
